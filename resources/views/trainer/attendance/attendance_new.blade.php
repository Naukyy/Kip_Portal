...
@keyframes slideIn { from { opacity:0; transform:translateX(-30px) scale(0.95); } to { opacity:1; transform:none; } }
...
.btn-start-class { background: linear-gradient(135deg,#3b82f6,#8b5cf6); box-shadow: 0 0 30px rgba(99,102,241,.5); transition: all .3s ease; }
...
<div class="p-4 md:p-6 max-w-7xl mx-auto w-full"
  ...
  {{-- ── LOCKED OVERLAY (Belum Mulai) ───────────────── --}}
  <div x-show="classState==='pending'"
    ...
    <div class="bg-white dark:bg-[#111827] rounded-2xl border border-gray-200 dark:border-gray-800 shadow-lg overflow-hidden flex flex-col">
      ...
      <div class="p-4 space-y-2 min-h-[200px] flex-1">
        <template x-if="doneStudents.length === 0">
          <div class="flex flex-col items-center justify-center py-10 text-center text-gray-400 dark:text-gray-600"><p class="text-sm">Belum ada yang diabsen</p></div>
        </template>
        ...
        <template x-for="s in doneStudents" :key="s.id">
          <div class="student-row entering flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800">
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
              <span :class="'badge-' + s.status" class="px-2 py-0.5 rounded-full text-xs font-semibold" x-text="statusLabels[s.status]"></span>
      ...
      <div x-show="classState==='in_progress'" class="p-4 border-t border-gray-100 dark:border-gray-800">
        <button @click="confirmEnd=true" :disabled="pendingStudents.length > 0" :class="pendingStudents.length === 0 ? 'bg-gradient-to-r from-red-500 to-pink-600 hover:shadow-lg hover:shadow-red-500/30 hover:scale-[1.02]' : 'bg-gray-100 dark:bg-gray-800 text-gray-400 cursor-not-allowed'" class="w-full py-3 rounded-xl font-semibold text-sm text-white transition-all duration-200 flex items-center justify-center gap-2">
      ...
      <div x-show="classState==='completed'" class="p-4 border-t border-gray-100 dark:border-gray-800">
        <div class="flex items-center gap-2 justify-center text-green-600 dark:text-green-400 text-sm font-semibold"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg> Kelas selesai — Data dikunci</div>
        <a href="{{ route('trainer.attendance.index', ['date'=>$date->toDateString()]) }}" class="mt-3 flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-primary-50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 text-sm font-medium hover:bg-primary-100 dark:hover:bg-primary-900/50 transition-colors">← Kembali ke Jadwal</a>
      </div>
    </div>
  </div>
    ...
    <div @click.stop class="bg-white dark:bg-[#111827] rounded-2xl border border-gray-200 dark:border-gray-700 shadow-2xl p-6 max-w-md w-full" x-transition>
      ...
      <div class="flex items-center gap-3 mb-3">
        <div class="w-10 h-10 rounded-full bg-yellow-100 dark:bg-yellow-900/40 flex items-center justify-center">
          <svg class="w-5 h-5 text-yellow-600 dark:text-yellow-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        </div>
        <div>
      ...
      <div class="space-y-4">
          ...
          <div class="mt-1 flex gap-2">
            <button type="button" @click="moveStatusValue='sakit'" class="flex-1 px-3 py-2 rounded-xl border text-sm font-semibold transition-colors"
                    :class="moveStatusValue==='sakit'
                      ...
                      : 'bg-white dark:bg-[#111827] border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300'">Sakit</button>
            <button type="button" @click="moveStatusValue='izin'" class="flex-1 px-3 py-2 rounded-xl border text-sm font-semibold transition-colors"
                    :class="moveStatusValue==='izin'
                      ...
                      : 'bg-white dark:bg-[#111827] border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300'">Izin</button>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select class="mt-1 w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#111827] text-sm text-gray-900 dark:text-white" x-model="moveTargetSessionTime">
  ...
  <div x-show="confirmEnd" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
    <div @click.stop class="bg-white dark:bg-[#111827] rounded-2xl border border-gray-200 dark:border-gray-700 shadow-2xl p-6 max-w-sm w-full" x-transition>
      <div class="flex items-center gap-3 mb-3">
        <div class="w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/40 flex items-center justify-center"><svg class="w-5 h-5 text-red-600 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg></div>
        <div><h4 class="font-bold text-gray-900 dark:text-white">Akhiri Kelas?</h4><p class="text-xs text-gray-500 dark:text-gray-400">Data presensi akan dikunci</p></div>
      </div>
...
function attendanceSPA() {
  ...
  return {
    ...
    init() {
      // minimal: dropdown berisi sesi meeting ini
    ...
    async submitMove() {
      ...
      try {
        ...
        // Update status di UI meeting asal menjadi izin/sakit
