...
<div class="p-6 md:p-8 max-w-7xl mx-auto w-full transition-colors duration-300">
    <div class="space-y-6" x-data="attendanceForm()">
        ...
        {{-- Date picker --}}
        <div class="bg-white dark:bg-[#111827] rounded-2xl border border-gray-200 dark:border-gray-800 p-6 shadow-lg">
            <form method="GET" class="flex flex-wrap gap-4 items-end">
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Tanggal</label>
                    <input type="date" name="date" value="{{ $date->toDateString() }}"
                           class="rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-[#111827] text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
        ...
        <form method="POST" action="{{ route('trainer.attendance.store') }}">
            ...
            <div class="bg-white dark:bg-[#111827] rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-lg">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50 dark:bg-[#1f2937]/50 text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide border-b border-gray-200 dark:border-gray-800">
                            <tr>
                                <th class="px-6 py-3 text-left font-semibold tracking-wider">Nama Siswa</th>
                                <th class="px-6 py-3 text-left font-semibold tracking-wider">Sesi</th>
                                <th class="px-6 py-3 text-center font-semibold tracking-wider">Status</th>
                                <th class="px-6 py-3 text-left font-semibold tracking-wider">Pengganti (jika cover)</th>
                                <th class="px-6 py-3 text-left font-semibold tracking-wider">Catatan</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800/60">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                <td class="px-6 py-4">
                                    ...
                                    <p class="text-xs text-gray-400 dark:text-gray-500">{{ $student->student_id }}</p>
                                </td>
                                    ...
                                    <div class="flex justify-center gap-1">
                                        ...
                                        <label class="cursor-pointer">
                                            ...
                                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg border text-xs font-bold
                                                         ...
                                                         peer-checked:bg-{{ $color }}-100 dark:peer-checked:bg-{{ $color }}-900/50
                                                         peer-checked:border-{{ $color }}-400
                                                         peer-checked:text-{{ $color }}-700 dark:peer-checked:text-{{ $color }}-400
                                ...
                                <td class="px-6 py-4">
                                    <select name="attendances[{{ $student->id }}][substitute_id]"
                                            class="text-xs rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-[#111827] text-gray-900 dark:text-white w-full focus:ring-2 focus:ring-blue-500 outline-none">
