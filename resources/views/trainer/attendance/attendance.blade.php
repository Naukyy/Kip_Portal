...
<div class="p-4 md:p-6 max-w-7xl mx-auto w-full"
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">
          {{ $meeting->session_time }} · {{ $date->locale('id')->translatedFormat('l, d F Y') }}
        </p>
      </div>
    </div>
  ...
  <div x-show="classState==='pending'"
    ...
    {{-- Blurred student list in background --}}
    <div class="filter blur-sm pointer-events-none select-none
                bg-white dark:bg-[#111827] rounded-2xl border border-gray-200 dark:border-gray-800 p-6">
      <div class="space-y-3">
    ...
    <div class="absolute inset-0 overlay-locked rounded-2xl flex flex-col items-center justify-center gap-5 z-10">
      ...
      <button @click="startClass()"
              :disabled="starting"
              class="btn-start-class px-8 py-4 rounded-2xl text-white font-bold text-base
                     flex items-center gap-3 disabled:opacity-60 disabled:cursor-not-allowed">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white dark:bg-[#111827] rounded-2xl border border-gray-200 dark:border-gray-800 shadow-lg overflow-hidden">
      <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <h3 class="font-semibold text-gray-900 dark:text-white text-sm">Belum Diabsen</h3>
        </div>
        <span class="text-xs font-semibold px-2 py-0.5 rounded-full
                     bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400"
              x-text="pendingStudents.length + ' siswa'"></span>
      </div>
      ...
      <div class="p-4 space-y-2 min-h-[200px]">
        ...
        <template x-for="s in pendingStudents" :key="s.id">
          <div class="student-row flex items-center gap-3 p-3 rounded-xl
            ...
            <div class="flex items-center gap-1 flex-shrink-0"
                 x-show="classState==='in_progress'">
              <button @click="setStatus(s, 'hadir')"
                      class="btn-status w-8 h-8 rounded-lg text-xs font-bold
                             bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400
                             border border-green-300 dark:border-green-700" title="Hadir">H</button>
              <button @click="setStatus(s, 'alpha')"
                      class="btn-status w-8 h-8 rounded-lg text-xs font-bold
    ...
    <div class="bg-white dark:bg-[#111827] rounded-2xl border border-gray-200 dark:border-gray-800 shadow-lg overflow-hidden flex flex-col">
      <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <h3 class="font-semibold text-gray-900 dark:text-white text-sm">Sudah Diabsen</h3>
        </div>
        <span class="text-xs font-semibold px-2 py-0.5 rounded-full
                     bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400"
              x-text="doneStudents.length + ' siswa'"></span>
      </div>
          ...
          <div class="student-row entering flex items-center gap-3 p-3 rounded-xl
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
              <span :class="'badge-'+s.status"
                    ...
                    x-text="statusLabels[s.status]"></span>
              <button x-show="classState==='in_progress'"
                      @click="undoStatus(s)"
      ...
      <div x-show="classState==='in_progress'" class="p-4 border-t border-gray-100 dark:border-gray-800">
        <button @click="confirmEnd=true"
                ...
                :class="pendingStudents.length === 0
                  ? 'bg-gradient-to-r from-red-500 to-pink-600 hover:shadow-lg hover:shadow-red-500/30 hover:scale-[1.02]'
                  : 'bg-gray-100 dark:bg-gray-800 text-gray-400 cursor-not-allowed'"
                class="w-full py-3 rounded-xl font-semibold text-sm text-white transition-all duration-200 flex items-center justify-center gap-2">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 10h6v4H9z"/>
          </svg>
          <span x-text="pendingStudents.length > 0
            ? pendingStudents.length + ' siswa belum diabsen'
            : 'End Class'"></span>
        </button>
      </div>
      ...
      <div x-show="classState==='completed'" class="p-4 border-t border-gray-100 dark:border-gray-800">
        ...
        <a href="{{ route('trainer.attendance.index', ['date'=>$date->toDateString()]) }}"
           class="mt-3 flex items-center justify-center gap-2 w-full py-2.5 rounded-xl
                  ...
                  text-sm font-medium hover:bg-primary-100 dark:hover:bg-primary-900/50 transition-colors">
          ← Kembali ke Jadwal
        </a>
      </div>
    </div>
  ...
  <div x-show="confirmEnd"
    ...
    <div @click.stop
         ...
         x-transition:enter-end="opacity-100 scale-100"
         class="bg-white dark:bg-[#111827] rounded-2xl border border-gray-200 dark:border-gray-700
                shadow-2xl p-6 max-w-sm w-full">
      <div class="flex items-center gap-3 mb-3">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400 mb-5">
        Apakah Anda yakin ingin mengakhiri kelas? Setelah dikonfirmasi, data presensi tidak dapat diubah.
      </p>
      <div class="flex gap-3">
        ...
        <button @click="endClass()"
                :disabled="ending"
                class="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-red-500 to-pink-600
                       text-white text-sm font-semibold hover:opacity-90 transition-opacity
                       disabled:opacity-60 disabled:cursor-not-allowed">
          <span x-text="ending ? 'Memproses...' : 'Ya, Akhiri'"></span>
        </button>
      </div>
    </div>
  </div>
  ...
  <div x-show="errorMsg"
       x-transition
       @click="errorMsg=''"
       class="fixed bottom-5 right-5 z-50 px-4 py-3 bg-red-600 text-white rounded-xl shadow-lg
              text-sm font-medium cursor-pointer max-w-xs"
       x-text="errorMsg"></div>
